...
class GLUETransformer(L.LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        # Set up the model config with the dropout rate
        self.config = AutoConfig.from_pretrained(model_name_or_path, num_labels=num_labels, hidden_dropout_prob=dropout_rate)
        self.model = AutoModelForSequenceClassification.from_pretrained(model_name_or_path, config=self.config)
        self.metric = evaluate.load("glue", task_name, experiment_id=datetime.now().strftime("%d-%m-%Y_%H-%M-%S"))
